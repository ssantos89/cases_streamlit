...
# Instânciando condicionais para validar pesquisa do sidebar
if gerar_graficos:
    ...
    if data.size > 0:
        ...
        with tab1:
            ...
            fig_close.update_layout(title=f'Histórico de Preços para {empresa_selecionada}', xaxis_title='Data', yaxis_title='Preço')
            st.plotly_chart(fig_close, use_container_width=True)

        with tab2:
            fig_volume = go.Figure()
            fig_volume.add_trace(go.Bar(x=data.index, y=data['Volume']))
            fig_volume.update_layout(title=f'Volume de Negociação para a empresa {empresa_selecionada}', xaxis_title='Data', yaxis_title='Volume')
        ...
        with tab3:
            ...
            fig_candle.update_layout(title=f'Gráfico de Velas para a empresa {empresa_selecionada}', xaxis_title='Data', yaxis_title='Preço')
