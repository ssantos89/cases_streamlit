...
# Definindo nome da página e título
...
# Criando sidebar da aplicação
with st.sidebar:
    uploaded_file = st.file_uploader('Escolha um Arquivo CSV', type='csv')
    if uploaded_file is not None:
        data_range = st.date_input('Informee o Período', [])
...
# Aplicando condicional para chamar aplicação
if uploaded_file is not None:
    ...
    if process_button and len(data_range) == 2:
        ...
        st.warning('Por favor selecione um período de datas válidas')
